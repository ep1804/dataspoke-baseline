...
Hooks are shell scripts that fire on specific Claude Code lifecycle events. Defined in `.claude/settings.json` under the `hooks` key.

| Hook | Event | Blocking | Purpose |
|------|-------|----------|---------|
| `validate-commit.sh` | PreToolUse (Bash) | Yes | Enforce Conventional Commits format; block `Co-Authored-By` and `Generated by` trailers |
| `protect-manifesto.sh` | PreToolUse (Edit\|Write) | Yes | Prevent accidental modification of `MANIFESTO_en.md` / `MANIFESTO_kr.md` |
| `post-compact-context.sh` | SessionStart (compact) | No | Reinject key project conventions (spec hierarchy, API routes, commit rules) after context compaction |
| `auto-format.sh` | PostToolUse (Edit\|Write) | No (async) | Auto-format Python (ruff) and TypeScript (prettier) files after edits |
| `spec-propagation-reminder.sh` | PostToolUse (Edit\|Write) | No | Remind to propagate changes through the spec hierarchy when editing `spec/` files |
