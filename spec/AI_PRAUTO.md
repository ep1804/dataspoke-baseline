...
[human adds prauto:ready]
    ...
    │       │       │
    │       │       └── approved + squash-pushed → removes prauto:review, adds prauto:done (on issue + PR)
    │       │
...
3. `squash_and_finalize_pr()` executes:
   ...
   h. Update PR title to the final squashed commit title
   i. Set `prauto:done` label on PR (remove `prauto:review`)
   j. Set `prauto:done` label on linked issue (remove `prauto:review`)
   k. Does **NOT** merge the PR or close the issue — leaves that for the human
